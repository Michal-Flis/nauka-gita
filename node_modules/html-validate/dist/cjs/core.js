...
function compatibilityCheck(name, declared, options) {
    ...
    const valid = satisfies__default['default'](current, declared);
    if (valid || silent) {
        return valid;
    }
    const text = [
...
ajv$1.addMetaSchema(require("ajv/lib/refs/json-schema-draft-06.json"));
/**
...
/* eslint-disable-next-line import/no-dynamic-require */
const entities$1 = require(path__default['default'].join(projectRoot, "elements/entities.json"));
...
class ConfigLoader {
    ...
    fromTarget(filename) {
        ...
        // eslint-disable-next-line no-constant-condition
        while (true) {
            const jsonFile = path__default['default'].join(current, ".htmlvalidate.json");
            if (fs__default['default'].existsSync(jsonFile)) {
                const local = this.configClass.fromFile(jsonFile);
                found = true;
                config = local.merge(config);
            }
            const jsFile = path__default['default'].join(current, ".htmlvalidate.js");
...
class Parser {
    ...
    getAttributeValueLocation(token) {
        if (!token || token.type !== exports.TokenType.ATTR_VALUE || token.data[1] === "") {
            return null;
        }
